...
class VideoService:
    ...
    def draw_actor(self, actor):
        ...
        if text == "*":
            pyray.draw_circle(actor.get_position().get_x(), actor.get_position().get_y(), 12.0, pyray.BLUE)

        if text == "#":
            pyray.draw_circle(actor.get_position().get_x(), actor.get_position().get_y(), 12.0, pyray.ORANGE)

        if text == "o":
            pyray.draw_circle(actor.get_position().get_x(), actor.get_position().get_y(), random.uniform(9.0, 15.0), pyray.RED)
    ...
    def _draw_grid(self):
        ...
        for y in range(0, self._height, self._cell_size):
            pyray.draw_line(0, y, self._width, y, pyray.GOLD)
        for x in range(0, self._width, self._cell_size):
            pyray.draw_line(x, 0, x, self._height, pyray.GOLD)